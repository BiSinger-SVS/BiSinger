...
class M4SingerBinarizer(MidiSingingBinarizer):
    ...
    def load_meta_data(self):
        ...
        with open(os.path.join(raw_data_dir, hparams['raw_json_fn']), 'r', encoding='utf-8') as f:
            lines = f.readlines()
            for line in lines:
                song_item = eval(line)
                item_name = raw_item_name = song_item['item_name']
    ...
    def get_pitch(item_name, wav, spec, ph, res):
        ...
        assert res['pitch_midi'].shape == res['midi_dur'].shape == res['is_slur'].shape, (item_name, res['pitch_midi'].shape, res['midi_dur'].shape, res['is_slur'].shape)

...
class BaseSVSInfer:
    ...
    def preprocess_word_level_input(self, inp):
        ...
        # #### note: missing phonemes: TH Y IH DH V W OY
        def replace_en_with_cn(ph_lst):
            ...
            for i in ph_lst:
                ...
                #     new_ph_lst.append('W')
                    ...
                    new_ph_lst.append("UW")
                elif i == "OY":
                    new_ph_lst.append("OW")
                else:
                    new_ph_lst.append(i)
            return new_ph_lst
        ...
        new_midi_dur_lst = []
        new_is_slur = []
        for ph, note, midi_dur, slur in zip(ph_lst, note_lst, midi_dur_lst, is_slur):
            if ph == "^":
                pass
            else:
                new_ph_lst.append(ph)
                new_note_lst.append(note)
                new_midi_dur_lst.append(midi_dur)
                new_is_slur.append(slur)
        ph_lst = new_ph_lst
        note_lst = new_note_lst
        midi_dur_lst = new_midi_dur_lst
        is_slur = new_is_slur
    ...
    def preprocess_input(self, inp, input_type="word"):
        ...
        if ret:
            ...
            with open(preprocess_ret_fn, "a") as f:
                f.write(f"{ph_seq}|{note_lst}|{midi_dur_lst}|{is_slur}\n")
        else:
    ...
    def input_to_batch(self, item):
        ...
        # lang = torch.LongTensor(item['lang'])[None, :hparams['max_frames']].to(self.device)
...
if __name__ == "__main__":
    ...
    for kind in kinds:
        for part in parts:
            ...
            save_path = f"/Netdata/2022/zhouhl/bisinger_mos/system-1/{kind}"
            preprocess_ret_fn = f"/Netdata/2022/zhouhl/bisinger_mos/system-1/{kind}.txt"

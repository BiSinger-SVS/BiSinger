...
if [ $type == CN ]
then
    sed -n '1,20000p' /NASdata/AudioData/mandarin/speech_synthesis/中文女生DB-4/$type/ProsodyLabeling/text.txt | while read line1 && read line2; do
        # 用awk分离出句子号和文本内容
...
then
    # sed -n '1,10000p' /NASdata/AudioData/mandarin/speech_synthesis/中文女生DB-4/$type/ProsodyLabeling/text.txt | while read line1 && read line2; do
    cat /NASdata/AudioData/mandarin/speech_synthesis/中文女生DB-4/$type/ProsodyLabeling/text.txt | while read line1 && read line2; do
        # 用awk分离出句子号和文本内容

...
class FastSpeech2Task(TtsTask):
    ...
    def add_pitch_loss(self, output, sample, losses):
        if hparams["pitch_type"] == "ph":
            ...
            losses["f0"] = (
                (
                    pitch_loss_fn(
                        output["pitch_pred"][:, :, 0], sample["f0"], reduction="none"
                    )
                    * nonpadding
                ).sum()
                / nonpadding.sum()
        ...
        if hparams["pitch_type"] == "cwt":
            ...
            if hparams["cwt_add_f0_loss"]:
                f0_cwt_ = self.model.cwt2f0_norm(
                    cwt_pred, f0_mean_pred, f0_std_pred, mel2ph
                )
                self.add_f0_loss(
                    f0_cwt_[:, :, None], f0, uv, losses, nonpadding=nonpadding
                )
        elif hparams["pitch_type"] == "frame":
            self.add_f0_loss(
                output["pitch_pred"], f0, uv, losses, nonpadding=nonpadding
            )
    ...
    def plot_mel(self, batch_idx, spec, spec_out, name=None):
        ...
        self.logger.experiment.add_figure(
            name, spec_to_figure(spec_cat[0], vmin, vmax), self.global_step
        )
    ...
    def plot_pitch(self, batch_idx, sample, model_out):
        ...
        if hparams["pitch_type"] == "cwt":
            ...
            self.logger.experiment.add_figure(
                f"cwt_{batch_idx}", spec_to_figure(cwt[0]), self.global_step
            )
            # f0
            f0_pred = cwt2f0(
                cwt_spec,
    ...
    def after_infer(self, predictions):
        if self.saving_result_pool is None and not hparams["profile_infer"]:
            self.saving_result_pool = Pool(
                min(int(os.getenv("N_PROC", os.cpu_count())), 16)
            )
        ...
        for num_predictions, prediction in enumerate(t):
            ...
            if self.phone_encoder is not None and "txt_tokens" in prediction:
                str_phs = self.phone_encoder.decode(
                    prediction["txt_tokens"], strip_padding=True
                )
            ...
            if not hparams["profile_infer"]:
                ...
                os.makedirs(
                    os.path.join(hparams["work_dir"], "P_mels_npy"), exist_ok=True
                )
                os.makedirs(
                    os.path.join(hparams["work_dir"], "G_mels_npy"), exist_ok=True
                )
                ...
                if mel_gt is not None and hparams["save_gt"]:
                    ...
                    if hparams["save_f0"]:
                        ...
                        if (
                            hparams.get("pe_enable") is not None
                            and hparams["pe_enable"]
                        ):
                        ...
                        plt.tight_layout()
                        plt.savefig(
                            f"{gen_dir}/plot/[F0][{item_name}]{text}.png", format="png"
                        )
                        plt.close(fig)
                ...
                self.stats["gen_wav_time"] += (
                    len(wav_pred) / hparams["audio_sample_rate"]
                )
    ...
    def save_result(
        ...
        if mel2ph is not None and str_phs is not None:
            ...
            dur = mel2ph_to_dur(torch.LongTensor(mel2ph)[None, :], len(decoded_txt))[
                0
            ].numpy()

...
class M4SingerBinarizer(MidiSingingBinarizer):
    ...
    def load_meta_data(self):
        ...
        with open(
            os.path.join(raw_data_dir, hparams["raw_json_fn"]), "r", encoding="utf-8"
        ) as f:
            lines = f.readlines()
            for line in lines:
                song_item = eval(line)
                ...
                self.item2wavfn[item_name] = (
                    f"{raw_data_dir}/{singer}#{song_name}/{sent_id}.wav"
                )
                self.item2txt[item_name] = song_item["txt"]
                ...
                self.item2sstype[item_name] = [
                    song_item["speechsing"]
                ]  # 0表示speech，1表示singing，2表示speech转singing
    ...
    def get_pitch(item_name, wav, spec, ph, res):
        ...
        res["lang"] = np.asarray(M4SingerBinarizer.item2lang[item_name])
        res["speechsing"] = np.asarray(
            M4SingerBinarizer.item2sstype[item_name]
        )  # <class 'numpy.ndarray'> (1,) [0]

        assert (
            res["pitch_midi"].shape
            == res["midi_dur"].shape
            == res["is_slur"].shape
            == res["lang"].shape
        ), (
    ...
    def process_item(
        ...
        try:
            ...
            if binarization_args["with_txt"]:
                ...
                if binarization_args["with_align"]:
                    cls.get_align(
                        M4SingerBinarizer.item2ph_durs[item_name],
                        mel,
                        phone_encoded,
                        res,
                    )
